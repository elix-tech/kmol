...
echo "     docker run --rm -ti --gpus=all --user \$(id -u):\$(id -g) -e MPLCONFIGDIR=/opt/elix/kmol/data/.mplconfig -v ./data:/opt/elix/kmol/data elix-kmol:1.1.5 {job} {path_to_config}"
echo
echo "   - Running an interactive shell in the same environment"
echo "     docker run --rm -ti --gpus=all --user \$(id -u):\$(id -g) -e MPLCONFIGDIR=/opt/elix/kmol/data/.mplconfig -v ./data:/opt/elix/kmol/data elix-kmol:1.1.5"
echo
