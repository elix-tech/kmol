...
docker tag "${IMAGE_ID}" "elix-kmol:1.1.9.1"

echo
echo " -o-  Launch examples -o- "
echo
echo "   - Simplest command (start kmol {job} {path_to_config} in a container)"
echo "     docker run --rm -ti --gpus=all --user \$(id -u):\$(id -g) -e MPLCONFIGDIR=/opt/elix/kmol/data/.mplconfig -v ./data:/opt/elix/kmol/data elix-kmol:1.1.9.1 {job} {path_to_config}"
echo
echo "   - Running an interactive shell in the same environment"
echo "     docker run --rm -ti --gpus=all --user \$(id -u):\$(id -g) -e MPLCONFIGDIR=/opt/elix/kmol/data/.mplconfig -v ./data:/opt/elix/kmol/data elix-kmol:1.1.9.1"
echo
