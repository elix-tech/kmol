...
from kmol.core.exceptions import CollaterError
...
def collater(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    filtered_items = [item for item in items if item is not None and item.x.size(0) <= max_node]
    if len(filtered_items) < len(items):
        raise CollaterError(f"[ERROR] A molecule exceeds specified max_node {max_node}. Please increase the max_node.")
    for it in filtered_items:
        if it.x.size(0) > 512:
            logging.warning(f"\n{it.smiles} exceeds 512")
